...
def when_the_nurse_post_to_the_product_resource_url_anoncare_api_assessments(step):
    ...
    world.assessment_post_response = world.app.post(world.assessment_post_uri, data=json.dumps(world.assessment))
    print 'HELLO'
@step(u'Then  the nurse should get a \'([^\']*)\' response')
def then_the_nurse_should_get_a_group1_response(step, expected_status_code):
    assert_equals(str(world.assessment_post_response.status_code), expected_status_code)
...
def and_the_nurse_get_a_field_status_containing_group1(step, expected_status):
    world.assessment_post_resp = json.loads(world.assessment_post_response.data)
    assert_equals(world.assessment_post_resp['status'], expected_status)
...
def and_the_nurse_get_a_field_message_containing_group1(step, expected_message):
    world.assessment_post_resp = json.loads(world.assessment_post_response.data)
    assert_equals(world.assessment_post_resp['message'], expected_message)